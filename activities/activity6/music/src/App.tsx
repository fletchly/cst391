...
const App = () => {
    ...
    const renderedList = () => {
        return albumList.map((album) => {
            return (<Card key={album.title} albumTitle={album.title} albumDescription={album.description} buttonText='OK'
                          imageUrl={album.image}/>);
        });
    };
    return <div className='container d-flex flex-wrap'>{renderedList()}</div>;
};
export default App;