      ...
      <div className={"p-2"}>
        <form onSubmit={handleSubmit}>
          <div className={"flex flex-row space-x-2 items-stretch"}>
            <input
              className={
                "w-50 border-b-2 border-b-gray-200 text-gray-800 focus:border-b-gray-800 transition-colors truncate outline-none"
              }
              type={"text"}
              name={"title"}
            ...
            <button
              type={"submit"}
              className={
                "hover:inset-shadow-xs cursor-pointer text-xl text-gray-500 active:text-white active:bg-gray-800 rounded-md bg-gray-200 px-2 py-1 transition-all"
              }
            >
            ...
            {preview ? (
              <div
                dangerouslySetInnerHTML={{
                  __html: MarkdownService.parseMd(formData.content),
                }}
              />
            ) : (
              <textarea
                className={
                  "block w-full field-sizing-content resize-none outline-none font-mono"
                }
                name={"content"}
